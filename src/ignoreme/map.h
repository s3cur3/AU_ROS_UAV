...
private:
    // define a 2-D array of grid_square using the boost libraries
    typedef boost::multi_array< grid_square, 1 > grid;
    typedef grid::index index; // for indexing the multi_array
    typedef grid::iterator outer_it;
    typedef grid::subarray< 1 >::type::iterator inner_it;
        
    grid * the_map; // a 2-D vector array of grid squares
    double width;
    double height;
    double resolution;
...
map::map( double width_of_field, double height_of_field, double map_resolution )
{
    ...
    the_map = new grid(boost::extents[ squares_wide ][ squares_high ]);
}
